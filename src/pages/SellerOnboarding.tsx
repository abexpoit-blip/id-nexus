...
import { ArrowLeft, ArrowRight, CheckCircle2, Loader2, Sparkles, Store, Upload, User2 } from "lucide-react";
import { toast } from "sonner";

interface Category { id: string; name: string; slug: string; price_bdt: number; }
        ...
        <Card className="border-primary/30 bg-gradient-card p-6 md:p-8" style={{ boxShadow: "0 0 40px -16px hsl(var(--primary) / 0.5)" }}>
          ...
          {stepIdx === 0 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Choose the seller name buyers will see.</p>
              <div>
          ...
          {stepIdx === 1 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Which category will you upload first? You can change it any time.</p>
              {categories.length === 0 ? (
                <div className="rounded-lg border border-border/60 bg-card/40 p-6 text-center text-sm text-muted-foreground">No active categories yet.</div>
              ) : (
          ...
          {stepIdx === 2 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">All set. Your CSV/XLSX file should have these columns:</p>
              <div className="rounded-lg border border-border/60 bg-card/40 p-4">
                <div className="font-mono text-sm">
                  <span className="text-primary">uid</span>, <span className="text-primary">password</span>,
                  <span className="text-muted-foreground"> two_fa</span>, <span className="text-muted-foreground"> email</span>, <span className="text-muted-foreground"> email_password</span>
                </div>
                <div className="mt-2 text-xs text-muted-foreground"><strong className="text-foreground">uid</strong> & <strong className="text-foreground">password</strong> required, others optional.</div>
              </div>
              <div className="rounded-lg border border-success/40 bg-success/10 p-4 text-sm">
                <div className="font-semibold text-success">✓ Setup summary</div>
